...
async function getWidgetTitle() {
    ...
    alert.addTextField(TEXTS.titlePicker.placeholder, fm.fileExists(path) ? fm.readString(path) : "")
...
async function getStartDate() {
    ...
    datePicker.initialDate = fm.fileExists(path) ? new Date(fm.readString(path)) : new Date()
...
async function extractRealColors(img) {
    ...
    // 等待颜色提取完成
    let colors = null
    for (let i = 0; i < 30; i++) {  // 最多等待 3 秒
        await new Promise(r => Timer.schedule(100, false, r))
        let result = await webView.evaluateJavaScript('window.extractedColors || null')
        if (result) {
...
const DEFAULT_PALETTES = [
    ...
    ["#a18cd1", "#fbc2eb", "#f5576c"],
    ["#1e3c72", "#2a5298", "#4776e6"]
]
...
async function createMeshGradientBackground(colors, width, height) {
    ...
    <body style="margin:0;padding:0;">
        <canvas id="canvas" width="${width * scale}" height="${height * scale}"></canvas>
        <script>
    ...
    for (let i = 0; i < 30; i++) {
        await new Promise(r => Timer.schedule(100, false, r))
        let result = await webView.evaluateJavaScript('window.meshGradientData || null')
        if (result) {
            base64Data = result
            break
        }
